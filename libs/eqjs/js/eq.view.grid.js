;(function ($, window) {
    ...
    EQ.view.grid = {
        ...
        init: function (options) {
            ...
            if (typeof (options.applyQueryFilterOnStart) === "undefined") {
                options.applyQueryFilterOnStart = true;
			}
            ...
            EQ.client.onInit = function () {
                if (options.applyQueryFilterOnStart) {
            ...
            this.applyQueryFilterUrl = options.applyQueryFilterUrl || EQ.core.combinePath(EQ.client.serviceUrl, "ApplyQueryFilter");
            this.applyQueryFilterOldUrl = EQ.core.combinePath(EQ.client.serviceUrl, "ApplyFilter");
            ...
            if (!this._funcs.applyQueryFilterButtonClick) {
                this._funcs.applyQueryFilterButtonClick = function () {
                    self.applyQueryFilter({ page: 1 });
                };
            }

            if (EQ.client.controls.FBWidget) {
                EQ.client.controls.FBWidget.FilterBar("option", "applyFilterCallback", this._funcs.applyQueryFilterButtonClick);
            }
        ...
        applyQueryFilter: function (options) {
            var self = this;
            ...
            $.ajax({
                                ...
                                if (paging.pageCount > 1) {
                                    ...
                                    if (pageNavigatorPlaceholder.length > 0) {
                                        pageNavigator = EQ.view.renderPageNavigator({
                                            ...
                                            pageCount: paging.pageCount,
                                            pageSelectedCallback: function (pageNum) {
                                                self.applyQueryFilter({ page: pageNum });
                                            }
                                        });
                ...
                error: function (statusCode, errorMessage, operation) {
                    if (statusCode.status === 404 && !self._useOldUrl) {
                        self._useOldUrl = true;
                        self.applyQueryFilterUrl = self.applyQueryFilterOldUrl;
                        self.applyQueryFilter();
                    }
                    else {
                        resultProgressIndicator.remove();
